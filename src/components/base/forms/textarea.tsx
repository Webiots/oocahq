...
function TextArea(match: {focus?:boolean, hover?:boolean, disabled?:boolean, error?:boolean, warning?:boolean, success?:boolean, wi?:boolean, icon?:boolean }) {
  return (
    <FormGroup className={`textarea-shadow ${match.focus?'focus':''} ${match.hover?'hover':''}
                 ${match.disabled?'disabled':''} 
                 ${match.error?'is-invalid':''} 
                 ${match.warning?' valid-warning':''}
                 ${match.success?' is-valid':''}`} >
    <Input type="textarea" rows="6" placeholder="Type here.." 
    className={`${match.disabled?'disabled':''} 
                ...
                ${match.success?'is-invalid' : match.wi ? 'without-icon': '' }`} />
    ...
    {(!match.error && !match.warning)?<span className="caption">180 Characters</span> :''}
    </FormGroup>
